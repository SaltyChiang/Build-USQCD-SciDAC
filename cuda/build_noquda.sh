...
if [ ${BUILD_CHROMA_QUDA} -gt 0 ]; then
    BUILD_QMP=$((${BUILD_CHROMA_QUDA} > ${BUILD_QMP} ? ${BUILD_CHROMA_QUDA} : ${BUILD_QMP}))
    BUILD_QDPXX=$((${BUILD_CHROMA_QUDA} > ${BUILD_QDPXX} ? ${BUILD_CHROMA_QUDA} : ${BUILD_QDPXX}))
    BUILD_QUDA=$((${BUILD_CHROMA_QUDA} > ${BUILD_QUDA} ? ${BUILD_CHROMA_QUDA} : ${BUILD_QUDA}))
fi

if [ ${BUILD_CHROMA_QUDA_JIT} -gt 0 ]; then
    BUILD_QMP=$((${BUILD_CHROMA_QUDA_JIT} > ${BUILD_QMP} ? ${BUILD_CHROMA_QUDA_JIT} : ${BUILD_QMP}))
    BUILD_QDPXX=$((${BUILD_CHROMA_QUDA_JIT} > ${BUILD_QDPXX} ? ${BUILD_CHROMA_QUDA_JIT} : ${BUILD_QDPXX}))
    BUILD_QDP_JIT=$((${BUILD_CHROMA_QUDA_JIT} > ${BUILD_QDP_JIT} ? ${BUILD_CHROMA_QUDA_JIT} : ${BUILD_QDP_JIT}))
    BUILD_QUDA_JIT=$((${BUILD_CHROMA_QUDA_JIT} > ${BUILD_QUDA_JIT} ? ${BUILD_CHROMA_QUDA_JIT} : ${BUILD_QUDA_JIT}))
fi
...
echo "BUILD_CHROMA_QUDA=${BUILD_CHROMA_QUDA}"
echo "BUILD_CHROMA_QUDA_JIT=${BUILD_CHROMA_QUDA_JIT}"
...
build ${BUILD_CHROMA_JIT} ${JOBS} chroma-jit \
    cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=${BUILD_SHAREDLIB} \
    -DChroma_ENABLE_OPENMP=ON -DChroma_ENABLE_JIT_CLOVER=ON \
    -DQMP_DIR=${DST}/qmp/lib/cmake/QMP -DQDPXX_DIR=${DST}/qdp-jit/lib/cmake/QDPXX \
...
build ${BUILD_CHROMA_QUDA} ${JOBS} chroma-quda \
    cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=${BUILD_SHAREDLIB} \
    ...
    -DCMAKE_INSTALL_RPATH=${DST}/chroma-quda/lib -DCMAKE_INSTALL_RPATH_USE_LINK_PATH=True \
    -DCMAKE_INSTALL_PREFIX=${DST}/chroma-quda ${SRC}/chroma

build ${BUILD_CHROMA_QUDA_JIT} ${JOBS} chroma-quda-jit \
    cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=${BUILD_SHAREDLIB} \
    ...
    -DCMAKE_INSTALL_RPATH=${DST}/chroma-quda-jit/lib -DCMAKE_INSTALL_RPATH_USE_LINK_PATH=True \
    -DCMAKE_INSTALL_PREFIX=${DST}/chroma-quda-jit ${SRC}/chroma
